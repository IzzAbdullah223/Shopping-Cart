...
function Strategy(){
    ...
    useEffect(()=>{
        const fetchData = async ()=>{
            const popularResponse = await fetch(`https://api.rawg.io/api/games?genres=strategy&key=${Apikey}`)
            const result = await popularResponse.json()

           const ratingResponse = await fetch(`https://api.rawg.io/api/games?genres=strategy&ordering=-rating&key=${Apikey}`)
           const result2 = await ratingResponse.json()

           const releaseResponse = await fetch(`https://api.rawg.io/api/games?genres=strategy&ordering=released&key=${Apikey}`)
