...
function Puzzle(){
    ...
    useEffect(()=>{
        const fetchData = async ()=>{
            const popularResponse = await fetch(`https://api.rawg.io/api/games?genres=puzzle&key=${Apikey}`)
            const result = await popularResponse.json()

           const ratingResponse = await fetch(`https://api.rawg.io/api/games?genres=puzzle&ordering=-rating&key=${Apikey}`)
           const result2 = await ratingResponse.json()

           const releaseResponse = await fetch(`https://api.rawg.io/api/games?genres=puzzle&ordering=released&key=${Apikey}`)
