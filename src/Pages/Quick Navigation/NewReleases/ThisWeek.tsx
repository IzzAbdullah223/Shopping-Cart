...
const formattedNextWeek = nextWeek.toISOString().split("T")[0];
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const popularResponse = await fetch(`https://api.rawg.io/api/games?dates=${formattedToday},${formattedNextWeek}&key=${Apikey}`)
            const result = await popularResponse.json()

            const ratingResponse = await fetch(`https://api.rawg.io/api/games?dates=${formattedToday},${formattedNextWeek}&ordering=-rating&key=${Apikey}`)
            const result2 = await ratingResponse.json()

            const releaseResponse = await fetch(`https://api.rawg.io/api/games?dates=${formattedToday},${formattedNextWeek}&ordering=-released&key=${Apikey}`)
