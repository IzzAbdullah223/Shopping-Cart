...
function AllTime(){
    ...
    useEffect(()=>{
        const fetchData = async ()=>{
            ...
            const response2 = await fetch(`https://api.rawg.io/api/games?key=${Apikey}&page=2`)
            const result2 = await response2.json()
            setData([...result.results,...result2.results])   //
        }
        fetchData()
    ...
    function displayGames(){
        ...
        console.log(data[0].parent_platforms[0].platform.name)

        gameImage.forEach((gameCardImage,index)=>{
            gameCardImage.src=data[index].background_image
            gameName[index].textContent=data[index].name
          
            const Platforms:JSX.Element[]=[]

            for(let i=0;i<data[index].parent_platforms.length;i++){


                if(data[index].parent_platforms[i].platform.name === "PC")
                    Platforms.push(<Windows key={i}></Windows>)

                if(data[index].parent_platforms[i].platform.name === "PlayStation")
                     
                    Platforms.push(<Playstation key={i}></Playstation>)

                if(data[index].parent_platforms[i].platform.name === "Xbox")
    
                    Platforms.push(<Xbox key={i}></Xbox>)

                 if(data[index].parent_platforms[i].platform.name === "Nintendo")
           
                    Platforms.push(<Nintendo key={i}></Nintendo>)
    
    
                if(data[index].parent_platforms[i].platform.name === "Apple Macintosh")
